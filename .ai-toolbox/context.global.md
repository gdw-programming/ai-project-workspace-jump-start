...
- **Documentation Location Map**: Each location serves a distinct audience — `.ai-toolbox/` (excluding `.ai-toolbox/docs/`) is AI agent context only (rules, domain knowledge, commands, patterns, tools); `.ai-toolbox/docs/` is human-readable documentation about the context system itself, accessible in all modes; `/docs/` is human-facing project documentation (setup, architecture, testing, contributing); `README.md` is the first-contact entry point and becomes the project developer's primary project doc after initialization; additional documentation structure beyond `README.md` is the developer's choice
...
- **Context Routing**: When the user asks to add, move, or update a context rule, determine the correct location before writing: (1) machine-specific tools, environment, or personal work-style preferences → `context.local.md`; (2) format rules, writing conventions, or section-specific behaviors for a named domain → `domains/{name}.md`; (3) project facts, scope, stakeholders, or quality standards → `project/overview.md` or `project/standards.md`; (4) system-wide behaviors, maintenance rules, or routing logic → `context.global.md`. If the correct location is ambiguous, propose the location and rationale before writing.
